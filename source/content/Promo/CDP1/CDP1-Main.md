# ✅ CDP 1


```{toctree}
:maxdepth: 2
:caption: Apprenants
:hidden:

Etudiants-CDP1/Adam
Etudiants-CDP1/Anny
Etudiants-CDP1/Audrey
Etudiants-CDP1/Celika
Etudiants-CDP1/Christian
Etudiants-CDP1/Christophe
Etudiants-CDP1/Corrine
Etudiants-CDP1/Jana
Etudiants-CDP1/Kannel
Etudiants-CDP1/Mahran
Etudiants-CDP1/Ranya
Etudiants-CDP1/Salman
Etudiants-CDP1/Sophia
Etudiants-CDP1/Souadrou
Etudiants-CDP1/Soula
Etudiants-CDP1/Sylvia
Etudiants-CDP1/Yanis
Etudiants-CDP1/Yassine

```

<br>

<p class="p-emphase"><strong> 09/2024 ⇨ 03/2025 </strong> : <em>La Promo "crash-test"</em> 🤕 </p>

***

## Apprenants

<p class="p-emphase"> 17 pépites : <em>A jamais les premiers !</em></p>

::::{grid} 4

:::{grid-item-card}
:link: Etudiants/Adam.html
:class-header: bg-light
:columns: 3
**Adam**
^^^

```{image} Docs/Logos/Adam-logo.png
:width: 100%
```

+++
✅
:::

:::{grid-item-card}
:link: Etudiants/Anny.html
:class-header: bg-light
:columns: 3
**Anny**
^^^

<br>

```{image} Docs/Logos/Anny-logo.png
:width: 100%
```

+++
✅
:::

:::{grid-item-card}
:link: Etudiants/Audrey.html
:class-header: bg-light
:columns: 3
**Audrey**
^^^

```{image} Docs/Logos/Audrey-logo.png
:width: 90%
:align: center
```

+++
✅
:::

:::{grid-item-card}
:link: Etudiants/Celika.html
:class-header: bg-light
:columns: 3
**Celika** 
^^^

Logo

+++
❌
:::

::::

::::{grid} 4

:::{grid-item-card}
:link: Etudiants/Christian.html
:class-header: bg-light
:columns: 3
**Christian** 
^^^

Logo

+++
❌
:::

:::{grid-item-card}
:link: Etudiants/Christophe.html
:class-header: bg-light
:columns: 3
**Chrisophe** 
^^^

Logo

+++
❌
:::

:::{grid-item-card}
:link: Etudiants/Corrine.html
:class-header: bg-light
:columns: 3
**Corrine** 
^^^

```{image} Docs/Logos/Corrine-logo.png
:width: 90%
:align: center
```

+++
✅
:::

:::{grid-item-card}
:link: Etudiants/Jana.html
:class-header: bg-light
:columns: 3
**Jana** 
^^^

Logo

+++
❌
:::


::::

::::{grid} 4


:::{grid-item-card}
:link: Etudiants/Kannel.html
:class-header: bg-light
:columns: 3
**Kannel** 
^^^

Logo

+++
❌
:::

:::{grid-item-card}
:link: Etudiants/Mahran.html
:class-header: bg-light
:columns: 3
**Mahran** 
^^^

<br>

```{image} Docs/Logos/Mahran-logo.png
:width: 100%
:align: center
```

<br>

+++
✅
:::

:::{grid-item-card}
:link: Etudiants/Ranya.html
:class-header: bg-light
:columns: 3
**Ranya** 
^^^

Logo

+++
❌
:::

:::{grid-item-card}
:link: Etudiants/Salman.html
:class-header: bg-light
:columns: 3
**Salman** 
^^^

Logo

+++
❌
:::

::::


::::{grid} 4

:::{grid-item-card}
:link: Etudiants/Sophia.html
:class-header: bg-light
:columns: 3
**Sophia** 
^^^

Logo

+++
❌
:::

:::{grid-item-card}
:link: Etudiants/Souadrou.html
:class-header: bg-light
:columns: 3
**Soidrou** 
^^^

```{image} Docs/Logos/Soidrou-logo.png
:width: 90%
:align: center
```

+++
✅
:::

:::{grid-item-card}
:link: Etudiants/Soula.html
:class-header: bg-light
:columns: 3
**Soula** 
^^^

<br>
<br>

```{image} Docs/Logos/Soula-logo.png
:width: 100%
:align: center
```

+++
✅
:::

:::{grid-item-card}
:link: Etudiants/Sylvia.html
:class-header: bg-light
:columns: 3
**Sylvia** 
^^^

Logo

+++
❌
:::

::::

::::{grid} 4

:::{grid-item-card}
:link: Etudiants/Yanis.html
:class-header: bg-light
:columns: 3
**Yanis** 
^^^

Logo

+++
❌
:::

:::{grid-item-card}
:link: Etudiants/Yassine.html
:class-header: bg-light
:columns: 3
**Yassine** 
^^^

```{image} Docs/Logos/yassine-logo.png
:width: 80%
:align: center
```

+++
✅
:::

::::

## Résultats

::::{grid} 2

:::{grid-item}
:columns: 5

<br>

> Description de la promo

:::

:::{grid-item}
:columns: 7

<div style="position: relative; width: 100%; height: 0; padding-top: 100.0000%;
 padding-bottom: 0; box-shadow: 0 2px 8px 0 rgba(63,69,81,0.16); margin-top: 1.6em; margin-bottom: 0.9em; overflow: hidden;
 border-radius: 8px; will-change: transform;">
  <iframe loading="lazy" style="position: absolute; width: 100%; height: 100%; top: 0; left: 0; border: none; padding: 0;margin: 0;"
    src="https://www.canva.com/design/DAGmmdKMFIw/WZmJQHB0-AcCqpYK9-G5CA/view?embed" allowfullscreen="allowfullscreen" allow="fullscreen">
  </iframe>
</div>
<a href="https:&#x2F;&#x2F;www.canva.com&#x2F;design&#x2F;DAGmmdKMFIw&#x2F;WZmJQHB0-AcCqpYK9-G5CA&#x2F;view?utm_content=DAGmmdKMFIw&amp;utm_campaign=designshare&amp;utm_medium=embeds&amp;utm_source=link" target="_blank" rel="noopener">Design par Vdeguin</a> 

:::

::::


## Souvenirs



```{image} Docs/Photos/Emy-apprenants-CDP1.jpg
:width: 80%
:align: center

```



